...
from chemdataextractor.scrape.entity import Entity, DocumentEntity
from chemdataextractor.scrape.fields import StringField, EntityField, UrlField
from chemdataextractor.scrape.scraper import RssScraper, SearchScraper, UrlScraper
...
class RscSupportingInfo(Entity):
    url = UrlField('a[href^="http://www.rsc.org/suppdata/"]::attr("href")')
    title = StringField('span[class="list__item-label"]', strip=True)
...
class RscLandingDocument(DocumentEntity):
    ...
    supporting_info = EntityField(RscSupportingInfo,'a[href^="http://www.rsc.org/suppdata/"]', all=True)
