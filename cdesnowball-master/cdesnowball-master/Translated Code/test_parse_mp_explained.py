...
'''
__future__ serves three purposes:
    1. to avoid confusing existing tools that analyze import statements and expect to find the modules they're importing
    2. To ensure that future statements run under releases prior to 2.1 at least yield runtime exceptions
    3. To document when imcompatible changes were introduced and when they will be - or were - made mandatory.'''




from lxml import etree
#I think this is why the debugging output is like this: 
'''
<mp_phrase>
  <cem>
    <name>3-Ethoxymethyl-2-(4-chlorophenyl)imino-2H-chromene</name>
    <label>E4</label>
  </cem>
  <mp>
    <value>84˜86</value>
...
'''Does basic configuration for the logging system by creating a StreamHandler (returns new instance of the StreamHandler class 

and if the stream is specified then the instance will use it for logging output; ptherwise, sys.stderr will be used) 

with a default Formatter and adding it to the root logger. 

The functions debug(), info(), warning(), error() and critical() will 

call basicConfig() automatically if no handlers are defined for the root logger.
...
class TestParseMp(unittest.TestCase):
    ...
    def do_parse(self, input, expected):
        s = Sentence(input)
        log.debug(s)
        '''debug provides us an understanding of what is going on with the code. It is the lowest logging level'''
        log.debug(s.tagged_tokens)
        '''tagged tokens is a method within the Sentence class'''
        result = next(mp_phrase.scan(s.tagged_tokens))[0]
        '''from mp.py mp_phrase = cem_mp_phrase | to_give_mp_phrase | obtained_mp_phrase
