...
class NeelTemperature(BaseModel):
    """A Neel temperatue measurement."""
    value = StringType()
    units = StringType(contextual=True)
    confidence = StringType()

class CurieTemperature(BaseModel):
    """ A curie temperature measurement """
    value = StringType()
    units = StringType(contextual=True)
    confidence = StringType()
...
class PhaseTransition(BaseModel):
    curie = ModelType(CurieTemperature)
    neel = ModelType(NeelTemperature)


class WeissConstant(BaseModel):
    """ A Weiss constant measurement"""
    value = StringType()
    units = StringType()

class CurieConstant(BaseModel):
    """ A Curie constant measurement"""
    value = StringType()
    units = StringType()

class MagneticMoment(BaseModel):
...
class Axis(BaseModel):
    """ A single axis, containing labels, marker indicators, units? """
    label = StringType()
    marker = StringType()
    units = StringType()

class MagneticSusceptibility(BaseModel):
    """ A magnetic susceptibility plot, and associated data"""
    weiss_constant = ModelType(WeissConstant)
    curie_constant = ModelType(CurieConstant)
    effective_magnetic_monent = ModelType(MagneticMoment)
    g_factor = StringType()

    # contextual elements
    apparatus = StringType(contextual=True)
    applied_field = StringType(contextual=True)
    applied_field_units = StringType(contextual=True)
    temperature_range = StringType(contextual=True)
    temperature_units = StringType(contextual=True)
...
class Figure(BaseModel):
    ...
    x_axes = ListType(ModelType(Axis), required=True)
    y_axes = ListType(ModelType(Axis), required=True)
    subplot_label = StringType()
...
class Compound(BaseModel):
    ...
    doping_levels = ListType(StringType())
    ...
    neel_temperatures = ListType(ModelType(NeelTemperature))
    curie_temperatures = ListType(ModelType(CurieTemperature))
    magnetic_susceptibility_data = ListType(ModelType(MagneticSusceptibility))
    figures = ListType(ModelType(Figure))
